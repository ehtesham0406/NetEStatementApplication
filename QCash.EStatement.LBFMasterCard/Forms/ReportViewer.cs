...
namespace StatementGenerator
{
    public partial class ReportViewer : Form
    {
        ...
        void grdEmailData_InitializeLayout(object sender, Infragistics.Win.UltraWinGrid.InitializeLayoutEventArgs e)
        {
            ...
            //
            e.Layout.Bands[0].Columns["EBALANCE"].Header.Caption = "END BALANCE";
            e.Layout.Bands[0].Columns["EBALANCE"].Width = 50;
            e.Layout.Bands[0].Columns["EBALANCE"].CellActivation = Activation.ActivateOnly;

            //
            e.Layout.Bands[0].Columns["TRNDESC"].Header.Caption = "TRN DESC";
            e.Layout.Bands[0].Columns["TRNDESC"].Width = 50;
            e.Layout.Bands[0].Columns["TRNDESC"].CellActivation = Activation.ActivateOnly;
                                ...
                                for (int i = 0; i < objESList.Count; i++)
                                {
                                    if (objESList[i].MAILADDRESS != "")
                                    {
                                        ...
                                        catch (Exception ex)
                                        {
                                            ...
                                            objLW.logTrace(_LogPath, "EStatement.log", System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:ss tt") + " : " + ex.Message);
                                    ...
                                    {
                                        txtAnalyzer.Invoke(_addText, new object[] { System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:ss tt") + " : " + "No Mail Address Found to send the Estatement " + objESList[i].FILE_LOCATION }); ;
                                        objLW.logTrace(_LogPath, "EStatement.log", System.DateTime.Now.ToString("MMMM dd, yyyy h:mm:ss tt") + " : No Mail Address Found to send the Estatement " + objESList[i].FILE_LOCATION);
